...
public class Task2_CountOccurrencesTest {
    ...
    void zeroOccurrencesWithDisjointCharSets(
            @ForAll @StringLength(min = 0, max = 200) @CharRange(from = 'a', to = 'm') String text,
            @ForAll @StringLength(min = 1, max = 20) @CharRange(from = 'n', to = 'z') String sub) {
    ...
    void countEqualsRepetitionFactor(
            @ForAll @StringLength(min = 1, max = 10) @CharRange(from = 'a', to = 'z') String sub,
    ...
    void wrappingWithNonMatchingCharsDoesNotChangeCount(
            @ForAll @StringLength(min = 0, max = 80) @CharRange(from = 'a', to = 'e') String text,
            @ForAll @StringLength(min = 1, max = 5) @CharRange(from = 'a', to = 'e') String sub,
            @ForAll @StringLength(min = 1, max = 20) @CharRange(from = 'v', to = 'z') String wrapper) {
        int baseline = StringUtils.countOccurrences(text, sub);
        int withPrefix = StringUtils.countOccurrences(wrapper + text, sub);
        int withSuffix = StringUtils.countOccurrences(text + wrapper, sub);

        assertThat(withPrefix).isEqualTo(baseline);
        assertThat(withSuffix).isEqualTo(baseline);
    }
